...
function randPic(){
  ...
  if ($setting == 'api.ohmyga.cn') {
    $output = 'https://www.nekopara.uk/pic_api/?rand='.$rand;
...
function Footer(){
  ...
  echo $footer.'<p>'.$copy.$content.'<br>由博主本人爆改主题<br><span id="cuckoo-copy">Theme <a href="https://github.com/bhaoo/cuckoo" target="_blank">Cuckoo</a> by <a href="https://dwd.moe/" target="_blank">Bhao</a>｜Powered By <a href="http://www.typecho.org" target="_blank">Typecho</a><br>猫猫服务器累计运行:'.$systemUptime.'喵~</span></p>';
}
...
function getServerUptime() {
    ...
    // 执行 uprecords 命令并获取输出  

    $output = shell_exec('uprecords');  
    ...
    // 获取倒数第三行（在PHP中，数组索引是从0开始的）  

    $uptimeLine = $lines[count($lines) - 4];  

  

    // 去除行首和行尾的空白字符  

    $uptimeLine = trim($uptimeLine);  

  

    // 如果倒数第三行不是我们要的uptime格式，则返回一个错误  

    if (!preg_match('/(\d+ days, \d+:\d+:\d+)/', $uptimeLine, $matches)) {  
    ...
    $uptimeString = $matches[1];  

  

    // 直接返回uptime字符串，或者你可以进一步处理它以去除秒数等  

    //return $uptimeString;  

  

    // 如果你想要返回一个格式化后的字符串（不包含秒）  

    list($daysPart, $timePart) = explode(', ', $uptimeString);  

    list($days, $hoursMinutes) = explode(' ', $daysPart);  
